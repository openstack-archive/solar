  ...
  config.vm.define "solar-dev", primary: true do |config|
    ...
    if provider == :docker
      ...
      config.trigger.after :up, :option => { :vm => 'solar-dev'} do
        docker_exec("solar-dev","rsyslogd >/dev/null 2>&1")
        docker_exec("solar-dev","sshd >/dev/null 2>&1")
    ...
    config.vm.define "solar-dev#{index}" do |config|
      ...
      if provider == :docker
        ...
        config.trigger.after :up, :option => { :vm => "solar-dev#{index}" } do
          docker_exec("solar-dev#{index}","rsyslogd >/dev/null 2>&1")
          docker_exec("solar-dev#{index}","sshd >/dev/null 2>&1")
