    ...
    def __call__(self, ctxt, *args, **kwargs):
        for sub in self._subscribers['before']:
            try:
                ...
                log.warning('Subscriber before %r failed with %s', sub, exc)
        try:
            ...
            for sub in self._subscribers['on_success']:
                try:
                    ...
                    log.warning('Subscriber on_success %r failed with %s', sub, exc)
            return rst
        except Exception as exc:
            for sub in self._subscribers['on_error']:
                try:
                    ...
                    log.warning('Subscriber on_error %r failed with %s', sub, exc)
            raise
        finally:
            for sub in self._subscribers['after']:
                try:
                    ...
                    log.warning('Subscriber after %r failed with %s', sub, exc)
