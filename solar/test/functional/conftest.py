...
def scheduler(request, scheduler_address):
    ...
    if 'timewatcher' in request.node.fixturenames:
        timewatcher = zerorpc_executor.Client(
            request.getfuncargvalue('timewatcher_address'))
    worker = workers.scheduler.Scheduler(tasks_client, timewatcher)
...
def timewatcher(tasks_address, scheduler_address, timewatcher_address):
    tasks = zerorpc_executor.Client(tasks_address)
    scheduler = zerorpc_executor.Client(scheduler_address)
    time_worker = workers.TimeWatcher(tasks, scheduler)
    gevent.spawn(
        zerorpc_executor.Executor(time_worker, timewatcher_address).run)
    return time_worker, zerorpc_executor.Client(timewatcher_address)
