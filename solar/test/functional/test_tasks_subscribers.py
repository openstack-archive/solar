...
def test_subscriber_message(tasks, tracer):
    assert tasks('echo', {}, 'test') == 'test'
    assert tracer.update.call_args_list == [call({}, 'test', 'test')]
...
def tasks_for_scheduler(tasks_worker, address):
    ...
    gevent.spawn(executor.run)
    return zerorpc_executor.AsyncClient(address)
